 ...
 * It supports:
 * - multilingual UI
 * - dynamic CoverageJSON loading
 * - dynamic parameter detection
 * - semantic descriptions from ontology-index.json
 * - clickable map cells with value + semantic definition + ontology link
 */
...
const I18N = {
  es: {
    ...
    appSubtitle:
      "Visor genérico basado en catálogo para observaciones, indicadores y coberturas categóricas codificadas como CoverageJSON.",
    language: "Idioma",
    coverage: "Cobertura",
    parameter: "Parámetro",
    opacity: "Opacidad",
    ...
    rawSemanticUri: "URI original",
    noData: "Sin datos",
    notAvailable: "No disponible",
    loading: "Cargando...",
    error: "Error",
    hint:
      "Para añadir una nueva observación o indicador, añade su archivo CoverageJSON a data/ y regístralo en data/catalog.json. El visor detectará los parámetros automáticamente."
  },

  en: {
    appTitle: "BioClima CoverageJSON Viewer",
    appSubtitle:
      "Generic catalogue-based viewer for observations, indicators and categorical coverages encoded as CoverageJSON.",
    language: "Language",
    coverage: "Coverage",
    parameter: "Parameter",
    opacity: "Opacity",
    ...
    rawSemanticUri: "Original URI",
    noData: "No data",
    notAvailable: "Not available",
    loading: "Loading...",
    error: "Error",
    hint:
      "To add a new observation or indicator, add its CoverageJSON file to data/ and register it in data/catalog.json. The viewer will detect the parameters automatically."
  },

  fi: {
    appTitle: "BioClima CoverageJSON -karttakatselin",
    appSubtitle:
      "Luettelopohjainen katselin CoverageJSON-muotoisille havainnoille, indikaattoreille ja luokitelluille kattavuuksille.",
    language: "Kieli",
    coverage: "Kattavuus",
    parameter: "Parametri",
    opacity: "Peittävyys",
    ...
    rawSemanticUri: "Alkuperäinen URI",
    noData: "Ei tietoja",
    notAvailable: "Ei saatavilla",
    loading: "Ladataan...",
    error: "Virhe",
    hint:
      "Lisää uusi havainto tai indikaattori lisäämällä CoverageJSON-tiedosto data/-kansioon ja rekisteröimällä se data/catalog.json-tiedostoon."
  },

  ro: {
    appTitle: "Vizualizator CoverageJSON BioClima",
    appSubtitle:
      "Vizualizator generic bazat pe catalog pentru observații, indicatori și acoperiri categorice codificate ca CoverageJSON.",
    language: "Limbă",
    coverage: "Acoperire",
    parameter: "Parametru",
    opacity: "Opacitate",
    ...
    error: "Eroare",
    hint:
      "Pentru a adăuga o observație sau un indicator nou, adăugați fișierul CoverageJSON în data/ și înregistrați-l în data/catalog.json."
  },

  zh: {
    appTitle: "BioClima CoverageJSON 查看器",
    appSubtitle:
      "基于目录的通用查看器，用于以 CoverageJSON 编码的观测、指标和分类覆盖数据。",
    language: "语言",
    coverage: "覆盖数据",
    parameter: "参数",
    opacity: "透明度",
    ...
    rawSemanticUri: "原始 URI",
    noData: "无数据",
    notAvailable: "不可用",
    loading: "加载中...",
    error: "错误",
    hint:
      "要添加新的观测或指标，请将 CoverageJSON 文件添加到 data/，并在 data/catalog.json 中注册。查看器会自动检测参数。"
  }
};
...
function cacheElements() {
  ...
  els.semanticDescription = document.getElementById("semanticDescription");
...
async function loadJson(url) {
  ...
  if (!response.ok) {
    throw new Error(`Cannot load ${resolved}: ${response.status} ${response.statusText}`);
  }
...
function updateUrlParams() {
  ...
  if (state.entry?.id) {
    params.set("coverage", state.entry.id);
  }

  if (state.currentParameterId) {
    params.set("parameter", state.currentParameterId);
  }
...
function parameterDisplayLabel(parameterId, parameter) {
  ...
  if (term) {
    return text(term.label) || parameterId;
  }
...
function updateSemanticPanel(parameterId) {
  ...
  if (uri) {
    const localOntologyUrl = `./ontology.html?uri=${encodeURIComponent(uri)}&lang=${encodeURIComponent(state.lang)}`;
    els.semanticLink.href = localOntologyUrl;
...
function extractCoverageValues(coverage, parameterId) {
  ...
  for (let row = 0; row < ny; row += 1) {
    for (let col = 0; col < nx; col += 1) {
      ...
      const value = normalizeValue(rawValues[flatIndex]);

      values[row * nx + col] = value;
    }
  }
...
function normalizeValue(value) {
  ...
  if (CONFIG.noDataValues.includes(numeric)) {
    return null;
  }
...
function ontologyTerm(uri) {
  if (!uri || !state.ontology?.terms) {
    return null;
  }

  if (state.ontology.terms[uri]) {
    return state.ontology.terms[uri];
  }

  const fragment = uri.includes("#") ? uri.split("#").pop() : uri.split("/").pop();

  return (
    state.ontology.terms[fragment] ||
    Object.values(state.ontology.terms).find((term) => {
      const id = term.id || term.uri || term["@id"];
...
function isCategoricalLayer() {
  ...
  const range = state.coverage?.ranges?.[state.currentParameterId] || {};

  if (state.entry?.kind === "categorical") {
    return true;
  }

  if (parameter.categories || parameter.categoryEncoding) {
    return true;
  }
...
function popupHtml(latlng) {
  ...
  const ontologyHref = uri
    ? `./ontology.html?uri=${encodeURIComponent(uri)}&lang=${encodeURIComponent(state.lang)}`
    : "#";

  const rawUriHref = uri || "#";
          ...
          uri
            ? `
              <tr>
                ...
                <td>
                  <a href="${escapeHtml(ontologyHref)}" target="_blank" rel="noopener">
                    ${escapeHtml(t("openOntology"))}
                  </a>
                  <br />
                  <small>
                    <a href="${escapeHtml(rawUriHref)}" target="_blank" rel="noopener">
                      ${escapeHtml(uri)}
                    </a>
                  </small>
                </td>
