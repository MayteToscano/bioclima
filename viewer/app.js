...
/*
 * Terms used to explain the metadata fields shown in the popup.
 * This means not only the selected scientific parameter is linked, but also
 * popup attributes such as Coverage, Value, Cell, Coordinates, Unit, etc.
 */
const VIEWER_FIELD_TERMS = {
  coverage: "https://maytetoscano.github.io/bblock-ebv-bioclima/vocab/#viewer_coverage",
  parameter: "https://maytetoscano.github.io/bblock-ebv-bioclima/vocab/#viewer_parameter",
  value: "https://maytetoscano.github.io/bblock-ebv-bioclima/vocab/#viewer_value",
  cell: "https://maytetoscano.github.io/bblock-ebv-bioclima/vocab/#viewer_cell",
  coordinates: "https://maytetoscano.github.io/bblock-ebv-bioclima/vocab/#viewer_coordinates",
  definition: "https://maytetoscano.github.io/bblock-ebv-bioclima/vocab/#viewer_definition",
  ontologyType: "https://maytetoscano.github.io/bblock-ebv-bioclima/vocab/#viewer_ontology_type",
  unit: "https://maytetoscano.github.io/bblock-ebv-bioclima/vocab/#viewer_unit",
  semanticUri: "https://maytetoscano.github.io/bblock-ebv-bioclima/vocab/#viewer_semantic_uri"
};
...
async function loadJson(url) {
  ...
  if (!response.ok) {
    throw new Error(`Cannot load ${resolved}: ${response.status}`);
  }
...
function updateSemanticPanel(parameterId) {
  ...
  if (semantic.href) {
    ...
    els.semanticTitle.innerHTML = `<a href="${escapeHtml(semantic.href)}" target="_blank" rel="noopener" title="${escapeHtml(semantic.uri || parameterId)}">${escapeHtml(title)}</a>`;
...
function extractCoverageValues(coverage, parameterId) {
  ...
  return {
    values,
    axes: { xValues, yValues, nx, ny, axisNames, shape }
  };
}
...
function fieldLabelLink(key) {
  const uri = VIEWER_FIELD_TERMS[key];
  const label = t(key);
  ...
  return `<a class="field-link" href="${escapeHtml(localOntologyHref(uri))}" target="_blank" rel="noopener" title="${escapeHtml(uri)}">${escapeHtml(label)}</a>`;
}
...
function popupHtml(latlng) {
  ...
  const parameterCell = semantic.href
    ? `<a class="ontology-link" href="${escapeHtml(semantic.href)}" target="_blank" rel="noopener" title="${escapeHtml(semantic.uri || parameterId)}">${escapeHtml(label)}</a>`
    : escapeHtml(label);
      ...
      <table class="popup-table">
        ...
        ${
          semantic.uri
            ? `
              <tr>
                <th>${fieldLabelLink("semanticUri")}</th>
                <td>
